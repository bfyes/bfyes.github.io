...
zensical: previews kill ## 实时预览文档站点（端口 8000）
	...
	@sleep 1.5 && uv run python scripts/patch_feature_blocks.py && uv run python scripts/patch_image_src.py && uv run python scripts/patch_page_info.py
	@wait
...
deploy: ## 本地构建并部署到 GitHub Pages（gh-pages 分支）
	...
	uv run python scripts/patch_page_info.py
