...
zensical: metadata contributions previews kill ## 实时预览文档站点（端口 8000）
...
deploy: ## 本地构建并部署到 GitHub Pages（gh-pages 分支）
	...
	cd site && rm -rf .git && git init && git add -A && git commit -m "deploy" --allow-empty && git -c http.postBuffer=524288000 push -f https://github.com/bfyes/bfyes.github.io.git main:gh-pages
