...
性能分析依赖可复现的测量和可验证的解释。CPU 矩阵乘法可以作为贯穿例子。先确定计时边界，再定位热点，检查计算、内存、线程和 NUMA 的影响，最后把改动与测量结果对应起来。热点是时间、CPU 周期或资源消耗占比很高的函数、循环或调用路径。IPC（instructions per cycle）表示平均每个处理器周期完成的指令数。FLOP（floating-point operation）表示一次浮点运算，矩阵乘法的一个乘加通常按两次 FLOP 统计。Lab 2 和 Lab 4 的优化报告可以沿用这一过程。[^nersc-perf]
...
性能数据必须先有问题定义。比较两个版本时，至少固定输入规模、数据类型、编译器和优化选项、线程数、绑核策略、NUMA 策略，以及是否把初始化、文件 I/O 和检查结果计入时间。第一次运行还可能包含动态链接、页面分配、缓存预热或 JIT，因此不能只取一次最快结果。[^benchmark]
...
这个数需要满足三个条件。FLOP 计数应对应实际算法，编译器没有因结果未使用而删除核心计算，输出通过独立的正确性检查。非方阵使用 $2MNK$；含激活、通信或 I/O 的程序不能只用某个 kernel 的 FLOP 数代表端到端吞吐。
...
计时函数应使用单调时钟。并行程序还要说明 barrier 是否位于计时区间内。测量用户等待时间时，应让所有参与者完成后再停止计时；测量单个 rank 的局部计算时，应单独标记并避免与前者混合。
...
![NERSC 的分层 Roofline 图同时绘出 L1、L2、片上缓存和 DDR 等不同存储层的带宽斜线，以及计算峰值横线。](assets/0712p/nersc-roofline-hierarchy.png)
...
!!! warning "不要从单个计数器直接下结论"
    ...
    “cache miss 高，所以要加预取”“IPC 低，所以要向量化”都不是完整推理。先确认事件发生在热点中，阅读该循环的访问步长、分支和数据依赖，再设计一个只改变一个因素的实验。优化后的同一组测量才是对假设的检验。
...
即使循环顺序合理，$B$ 或 $C$ 足够大时，下一次复用到同一行数据前，它们仍可能已经被逐出缓存。分块将 $M$、$N$、$K$ 三个维度划成 tile，使一个 $A$ tile、一个 $B$ tile 和一个 $C$ tile 在离开缓存前完成更多计算。分块减少的是向慢速层重复搬运的数据量，不是数学上的乘加次数。[^blas]
...
一个微内核通常让少量 $C$ 元素长时间留在寄存器中。以图中的 $4\times16$ 双精度输出 tile 为例，4 行向量累加器保存 $C$ 的部分结果；每次从 packed $B$ 面板加载向量，把一个 $A$ 标量广播后执行 FMA。这样的设计提高 $A$、$B$ 的寄存器级复用，但同时受寄存器数量、加载端口、指令延迟与目标 ISA 限制。[^intel-intrinsics]

![图中的 AVX-512 微内核把一个 4×16 的 C tile 保存在 8 个 ZMM 累加器中；每轮 K 循环广播 A 的标量、加载 B 的两个向量并累加。](assets/0712p/avx512-microkernel.png)

*微内核展示高性能 GEMM 中的 packing、向量寄存器和固定 tile。它依赖 AVX-512。[^intel-intrinsics]*
...
计时边界必须服务于问题。优化单个 kernel 时，先完成输入准备与预热，再在相同 stream、相同线程团队或相同 MPI 分配中计时；优化用户体验时，读取、传输、同步、输出和检查点都应纳入端到端时间。GPU kernel 使用 CUDA event，MPI 程序可使用 `MPI_Wtime` 并明确是否在 barrier 后开始。
...
一条完整的性能结论应说明哪个函数占时最多、循环如何访问数据或同步、哪项事件支持该观察、改动改变了什么，以及改动后时间和正确性如何变化。无法联系到具体循环的计数器，只能作为待验证线索，不能直接写成结论。
