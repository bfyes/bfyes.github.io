...
性能分析依赖可复现的测量和可验证的解释。CPU 矩阵乘法可以作为贯穿例子。先确定计时边界，再定位热点，检查计算、内存、线程和 NUMA（Non-Uniform Memory Access，非一致内存访问）的影响，最后把改动与测量结果对应起来。热点是时间、CPU 周期或资源消耗占比很高的函数、循环或调用路径。IPC（instructions per cycle）表示平均每个处理器周期完成的指令数。FLOP（floating-point operation）表示一次浮点运算，矩阵乘法的一个乘加通常按两次 FLOP 统计。Lab 2 和 Lab 4 的优化报告可以沿用这一过程。[^nersc-perf]
