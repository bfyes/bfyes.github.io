...
!!! question "思考题"
    ...
    为什么循环里存在写后读依赖时，向量化仍然可能可行；但读后写或写后写依赖时常常不行？

    ??? tip "答案"

        写后读表示本轮使用上一轮已经完成的值，若能保留原始输入序列，仍可按 SIMD 的分组语义处理。读后写会让后面的写覆盖后面要读的值，写后写会让同一位置的最终值取决于执行顺序。若并行执行改变结果，向量化就不合法，除非重写算法或使用特殊的依赖分析。

!!! question "思考题"

    `for i: a[i] = a[i-1] + b[i]` 为什么不能直接把不同 `i` 分给不同线程？

    ??? tip "答案"

        当前迭代读取前一个迭代写入的 `a[i-1]`，存在跨迭代依赖。若并行执行，写入和读取顺序不确定，结果也不确定。这类计算需要前缀扫描等专门算法，或改为可并行的数学形式。
...
!!! question "思考题"
    ...
    两个数组形状分别是 `(3, 1)` 和 `(1, 4)`。广播后结果形状是什么？这个机制为什么能避免显式复制？

    ??? tip "答案"

        结果形状是 `(3, 4)`。NumPy 沿大小为 1 的维度重复使用已有数据，而不是真的复制出完整矩阵。底层仍然按连续或步长方式读取原数组，因此表达式更短，也可能交给 BLAS 或向量化内核执行。但广播也可能隐藏较大的临时数组，复杂表达式仍要看内存和计算量。
...
!!! question "思考题"
    ...
    为什么归约操作的向量顺序会影响浮点结果？这不影响算法正确性时为什么仍然重要？

    ??? tip "答案"

        浮点加法不满足严格结合律，改变分组会改变舍入误差。数学上近似相等的结果，在并行或向量化后可能差几个低位。比较程序、回归测试或分布式训练时，如果假设逐位一致，就可能误判为错误。需要固定归约顺序、使用补偿求和，或在文档里说明容差。

!!! question "思考题"

    一条向量指令有 8 个 lane，程序为什么常常得不到 8 倍加速？

    ??? tip "答案"

        数据可能没有连续对齐，尾部不足 8 个元素，访存带宽跟不上，或者存在依赖和分支。向量寄存器只增加每条指令处理的数据量；要得到接近理想的收益，还需要数据布局、循环结构和计算强度配合。
...
!!! question "思考题"
    ...
    矩阵乘法分块为什么会提高性能？它减少了计算量还是改变了访存量？

    ??? tip "答案"

        分块通常不减少乘加次数，而是提高数据在寄存器和 cache 中的复用。一个 `A` 的 tile 或 `B` 的 tile 被载入后可以参与多个输出 tile 的计算，从而减少从低层存储重复取数据的次数。块太大放不进 cache，块太小复用不足，因此要结合矩阵形状和缓存容量调块大小。
...
!!! question "思考题"
    ...
    同一个向量化函数第一次运行很慢，之后明显变快。性能测试应该记录哪些环境因素？

    ??? tip "答案"

        至少记录输入规模、数据类型、初始化方式、编译选项、CPU 频率、线程绑定、cache 状态、计时范围和重复次数。第一次运行可能包含页错误、动态加载、分支预测冷启动和频率爬升。正式测量常用预热加多次采样，同时报告中位数、均值、最小值和波动范围。
