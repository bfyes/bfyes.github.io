...
第一个是前缀和，形成一条依赖链；第二个是归约。它们并非永远不能并行，而是需要不同算法：并行前缀和会分块再合并，归约会让每个工作者先算局部和、最后相加。这个区别很重要：**不能因为看到 `for` 就加并行指示。**
...
这里没有“神奇地消除计算”：我们只是把边界问题转成一次规则的 padding，把两重循环转成三个规则的切片。这正是向量化中最有价值的能力：改变数据的表示，使计算形状变规则。
...
更宽的单元需要更多电路和供电，部分 CPU 在高强度 AVX-512 下会降频（AVX-512 还被戏称为“大火炉” 🔥），内存带宽也可能先到极限。指令集版本只能说明有哪些工具，不能单独推出程序会快多少。
