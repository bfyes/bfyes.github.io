...
集群使用从连接和文件开始，再进入软件环境、编译和作业提交。这些环节前后相连，排查时仍可按边界逐项确认。

网络连接查看接口、路由和端口。远程登录认证查看主机密钥和用户密钥。程序启动时查看路径、权限和动态库；作业状态则回到 Slurm 的资源分配和日志。这些基础操作贯穿 Lab 0 和 Lab 1。[^slurm-overview]

!!! quote "Linux 101 的命令行建议（意译）"
...
!!! tip "排障记录"
    ...
    排障时保存完整命令、时间、主机名、退出码和标准错误。最后一行报错往往只说明程序在哪一步停止，不能说明原因。

    `ssh -vvv`、`module list`、`srun hostname`、`ldd` 和作业输出文件各自回答不同问题。先让一条命令回答一个问题，后面的判断才有依据。

## 从个人电脑到集群作业

一次提交到集群的程序，会先经过网络和认证，随后进入 Linux 环境、文件系统和调度器。前面的环节还没有通时，后面的检查通常得不到有效结果。
