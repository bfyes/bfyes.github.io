...
=== "1. 连到服务器"
    ...
    本机需要有可用接口、地址和路由；域名需要解析到正确地址；目标主机的 SSH（Secure Shell，安全远程登录协议）端口需要可建立 TCP（Transmission Control Protocol，提供可靠字节流的传输协议）连接。对应命令是 `ip route get`、`getent hosts`、`nc -vz`。
...
主机通过网络接口控制器（network interface controller，NIC，网络接口控制器）接入网络。Linux 将物理网卡、Wi-Fi、回环接口、容器 veth、VPN/TUN 接口都表示为网络接口。veth 是容器常用的一对虚拟以太网接口。VPN（Virtual Private Network，虚拟专用网络）通常会建立一条加密或隧道化的网络路径；TUN 是这种路径常使用的虚拟接口，用于承载 IP 包。接口是否存在、是否启用、有无地址、路由如何选择，是四个不同层次的事实。`iproute2` 的 `ip` 工具是 Linux 上检查这些信息的标准入口。[^iproute]
...
以太网帧在同一二层网络内按 MAC（Media Access Control，媒体访问控制）地址转发。二层网络可以先理解为一组由交换机直接连接、帧不必经过路由器就能转发的接口。MAC 地址是网卡在这一级使用的地址，用于定位接口。交换机从收到帧的源 MAC 学习端口位置；目的 MAC 已知时只向对应端口转发，未知时才复制到除入端口外的其他端口，这个动作称为泛洪。路由器则查看 IP（Internet Protocol，互联网协议）前缀和路由表，在不同网络之间选择下一跳，即这个 IP 包接下来应交给的路由设备或目标主机。MAC 和 IP 服务的范围不同，查看 `arp`/邻居表和查看路由表也因此回答不同问题。
...
集群常同时存在管理网络和计算网络。管理网络承载 SSH、部署、监控和带外管理。带外管理指独立于操作系统业务网络的管理员控制通道，常用于查看硬件状态或远程开关机。计算网络承载 MPI、RDMA（Remote Direct Memory Access，远程直接内存访问）或加速器间通信。能够 SSH 到登录节点，只能证明管理访问可用，不能证明计算节点的高速互连、GPU 互连或并行文件系统带宽正常。用户程序通常通过 MPI、NCCL（NVIDIA Collective Communications Library，NVIDIA 集合通信库）或系统库使用计算网络，而不是直接配置交换机。

## IP、子网与默认网关

IPv4 地址和 CIDR（Classless Inter-Domain Routing，无类别域间路由）前缀共同描述接口所在子网。例如 `192.168.136.23/24` 的前 24 位是网络部分，等价掩码为 `255.255.255.0`。目标地址与本机接口落在同一前缀时，主机通过链路层直接寻找目标。这里的链路层就是本地网段内依靠 MAC 地址和交换机发送帧的这一层。目标不在本地子网时，主机将 IP 包交给默认网关，也就是本机预先配置的出口路由器，再由路由器继续转发。IPv6 也使用前缀长度表达网络范围。[^rfc8200]
...
DNS（Domain Name System，域名系统）、TCP 和 SSH 处在不同层次。DNS 将主机名解析为地址；TCP 在两端建立可靠、按序的字节流；SSH 在 TCP 之上协商主机身份、用户认证和加密会话。DNS 失败时先检查名称解析；TCP 端口不可达时检查路由、防火墙或服务；网络连通后再处理密钥和用户认证。[^ssh-man]
...
绝对路径从 `/` 开始；相对路径以当前工作目录为基准。路径能否访问不只取决于最后一个文件，还取决于沿途每一级目录的执行权限。挂载点是把一个文件系统接到目录树某个位置的目录，例如外接磁盘或 NFS 目录会挂到 `/mnt/...` 或课程目录下。`findmnt -T` 可显示某个路径来自哪个挂载点，`namei -l` 可逐级检查路径权限。[^path-resolution]
...
*图中权限位只表达访问规则的一部分。ACL、挂载选项、SELinux/AppArmor 等由系统额外执行的安全策略，以及网络文件系统策略，还可能叠加额外限制。[^path-resolution]*
...
静态库通常以 `.a` 结尾，在链接时复制所需对象代码；共享库通常以 `.so` 结尾，由动态加载器在运行时寻找。动态加载器是程序启动时负责定位并装入共享库的系统组件。Linux 上的可执行文件和共享库通常采用 ELF（Executable and Linkable Format，可执行与可链接格式）。C++ 与 C、不同编译器版本或不同 C++ 标准库之间还会涉及 ABI（Application Binary Interface，应用二进制接口），即编译后的函数调用、对象布局和库链接约定是否兼容。Lab 1 自行构建 OpenMPI（开源 MPI 实现）、BLAS（Basic Linear Algebra Subprograms，基础线性代数子程序库）和 HPL（High Performance Linpack，高性能 Linpack 基准）时，应记录安装前缀、编译器、依赖库和配置参数。[^gcc-link]
...
Linux 动态加载器按 RPATH/RUNPATH（Runtime Search Path，运行时搜索路径）、`LD_LIBRARY_PATH`、系统缓存等规则寻找共享库。运行节点与登录节点的模块或挂载路径不同，会导致“登录节点能运行、计算节点不能运行”。在作业脚本中打印关键环境和 `ldd` 输出，能快速确认实际加载的库版本。[^ldso]
...
带外管理由 BMC（Baseboard Management Controller，主板管理控制器）提供独立于操作系统的硬件监控、远程控制台和电源管理。它面向管理员，不是普通用户的登录入口。业务网络故障或操作系统崩溃时，BMC 仍可能帮助管理员获取硬件日志和恢复主机。
