...
$\mathcal{L}_{\mathrm{LM}}$ 是语言模型训练损失，$t$ 是序列位置，$x_t$ 是第 $t$ 个真实 token，$x_{<t}$ 表示它左侧的历史 token，$p_	heta$ 是参数为 $	heta$ 的模型给出的条件概率。
...
*图下方的 `DP × TP × PP × EP (+ CP)` 表示这些并行维度可以组合，组合规模要与互连拓扑一起选择。图中的各列提供课程概览；实际显存、吞吐和通信量仍取决于模型形状与集群配置。*
...
$x_t$ 是第 $t$ 个 token 的隐藏向量，$W_{\mathrm{router}}$ 将它投影为 expert 分数，$\operatorname{topk}$ 保留分数最高的少数 expert，$g_{t,e}$ 是 token $t$ 分给 expert $e$ 的 gate 权重，$\operatorname{FFN}_e$ 是该 expert 的前馈网络。
...
低秩的含义来自这两次矩阵乘法。把一个 batch 放进线性层后，形状关系会更直观。设输入 `x` 的形状是 `[N,d]`，其中 `N` 是本次同时处理的 token 数。基座输出和 LoRA 增量都必须回到 `[N,k]`，才能逐元素相加。

```python
# W0: [k, d]，A: [r, d]，B: [k, r]
base = x @ W0.T          # [N, k]，冻结的基座路径
low  = x @ A.T           # [N, r]，先压到低秩空间
delta = low @ B.T        # [N, k]，再投回输出维度
y = base + scale * delta # [N, k]
```

全量微调需要更新 $W_0$ 的 $kd$ 个参数；LoRA 只更新 $A,B$ 的 $r(d+k)$ 个参数。若 $k=d=4096$、$r=16$，全量参数约 1678 万，LoRA 分支约 13.1 万。这个差异直接减少可训练参数、梯度和优化器状态，但前向仍要执行增量分支的两次小矩阵乘。
...
设有 $L$ 层、$H_{kv}$ 个 KV head、每个 head 维度为 $d$、上下文长度为 $S$。每个 token 在每层要保存一份 K 和一份 V，因此单请求缓存的**元素数**近似为

$$
N_{\mathrm{KV}}\approx 2\times L\times S\times H_{kv}\times d
$$

这里 $L$ 是层数，$S$ 是当前上下文 token 数，$H_{kv}$ 是每层保存的 K/V head 数，$d$ 是每个 head 的维度。前面的 2 表示 K 与 V 各保存一份。

若每个元素占 $b$ 个字节，缓存字节数再乘一次 $b$：

$$
\mathrm{KV\ bytes}\approx 2\times L\times S\times H_{kv}\times d\times b
$$

BF16 或 FP16 常取 $b=2$，FP32 取 $b=4$。多并发请求、长上下文和多轮对话会很快填满 GPU HBM。decode 每步算术量并不大，却反复读取越来越长的 cache，常呈现 memory-bound 或 I/O-bound 特征。
...
$\mathrm{NLL}$ 是负对数似然，$t$ 遍历计分位置，$p_	heta(x_t\mid x_{<t})$ 是模型给真实 token 的条件概率。
