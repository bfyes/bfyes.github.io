...
流水线分三段：prologue 先异步装好第一块；steady state 让算当前块与装下一块一直重叠；drain 处理已无新输入的最后一轮。stage 数越多、每块越小，重叠越平滑，但 shared memory 占用与调度复杂度也越高。能用 double buffering 的前提是有独立的异步 copy 路径（如 `LDGSTS`/TMA），否则仍会退化为同步等数据。
