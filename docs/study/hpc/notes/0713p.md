...
![课件以矩阵相加为例，将二维 block/thread 坐标对应到输出矩阵的位置。](assets/0713-morning/cuda-thread-mapping.png)
...
![Tiled GEMM 的层级：block 从 global memory 取块，warp 和线程继续把工作拆到 shared memory、寄存器和计算单元。](assets/0713/gpu-tiled-gemm.jpg)
...
![Volta/Ampere 的 MMA 指令由一个 warp 协作完成一个小矩阵乘加；图中展示了矩阵元素如何分散在 32 个 lane 的寄存器 fragment 中。](assets/0713/tensor-core-mma.jpg)
