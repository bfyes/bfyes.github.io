...
CUDA（Compute Unified Device Architecture，统一计算设备架构）将程序分为 host 与 device 两侧。host 准备数据、建立执行依赖并发射 kernel。device 用大量线程执行数据并行部分。编写 kernel 时需要明确线程负责的输出、输入如何进入设备、数据怎样在寄存器和 shared memory 中复用，以及同步发生在哪里。[^cuda-guide]
