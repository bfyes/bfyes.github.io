...
CUDA 将程序分为 host 与 device 两侧。host 准备数据、建立执行依赖并发射 kernel。device 用大量线程执行数据并行部分。编写 kernel 时需要明确线程负责的输出、输入如何进入设备、数据怎样在寄存器和 shared memory 中复用，以及同步发生在哪里。[^cuda-guide]
...
=== "3. Kernel 在 GPU 上执行"
    ...
    block 被调度到 SM，线程按 warp 执行。kernel 从 global memory 读取数据，可在 block 内使用 shared memory 和同步，再把输出写回 global memory。
...
一个可调试的最小程序包括分配 device 内存、复制输入、配置 grid 和 block、发射 kernel、检查错误、取得结果和释放资源。开发阶段可以逐步检查返回值。性能测量时要区分调试同步和算法本身需要的同步。[^cuda-runtime]
...
先保留一个 CPU 或 PyTorch 参考实现。先比较小形状、非整除形状、零长度边界、随机输入和极端值的输出，再扩展到性能输入。kernel 在优化前应有明确的误差判据，这样后续的加速比才有可比较的参考。

## Grid、Block 与 Thread

`__global__` 函数由 host 用 `<<<grid, block>>>` 启动。每个线程通过 `threadIdx`、`blockIdx` 和 `blockDim` 计算自己负责的逻辑位置。下面的一维 SAXPY 将第 $i$ 个输出交给一个线程。
...
Tensor Core 加速特定形状和精度组合的矩阵乘加。cuBLAS、cuBLASLt、CUTLASS 与 WMMA 提供不同的抽象层。它们对数据类型、布局、对齐、维度和累加精度都有明确约束。应先使用库实现建立正确、快速的基线，再判断自定义 kernel 是否确有无法表达的融合或布局需求。[^cublas]

许多路径允许 FP16、BF16、TF32 等输入，并以 FP32 或更高精度累加。实际行为取决于所用 API、GPU 架构和数学模式。科学计算和量化推理对误差的允许范围不同，选择数据格式时还要检查误差要求。
...
![图中展示 CUDA runtime 中 host、复制和 kernel 的时间关系。实际重叠需要没有数据依赖、使用合适 stream，并且硬件具有相应的 copy/compute 能力。](assets/0713p/cuda-runtime-async.jpg)

*异步 API 允许 host 提前提交工作。设备端是否真正发生重叠，仍要在实际时间线上验证。[^cuda-stream][^ncu]*
...
kernel launch 返回时，device 可能仍在使用其参数和缓冲区。主机端应在最后一次使用完成后再释放、覆盖或复用这些内存。事件既能表达依赖，也可用于测量同一 stream 内的设备时间。端到端墙钟时间还应保留 host 准备、排队与必要同步。
...
Triton 用 Python DSL 描述由 program instance 处理的 tensor block，并由编译器生成 GPU 代码。它适合快速迭代规则张量 kernel。程序员仍需明确 block 映射、mask、布局、数据类型和基准方法。Triton 中的 `tl.load`、`tl.store` 与 program id 同样会产生越界、跨步访问和寄存器压力问题，只是表达形式不同。[^triton]
