...
这些名词可以直接当"约定俗成的分层"来记：TMA 管搬运进 shared memory，WGMMA/MMA 管在片上完成矩阵乘加，epilogue 管把结果按想要的布局写出去。Lab 里用 CUTLASS 写高性能 GEMM 时，看到的 `tma`、`wgmma`、`epilogue`、`Ctate` 都属于这个分工。
