...
并行程序需要明确数据所有权、并发读写、同步点和跨节点传输。OpenMP 用于共享内存线程，MPI 用于独立进程间的消息传递，C++ 标准库提供线程和同步原语。Lab 1 的 HPL 使用 MPI，Lab 2 的 CPU kernel 使用线程与 SIMD，Lab 4 可能同时使用 MPI、OpenMP 和 GPU。[^openmp-spec][^mpi-standard]
