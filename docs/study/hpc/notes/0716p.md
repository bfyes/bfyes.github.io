...
!!! abstract "与后续实验的联系"
    ...
    Lab 5 的量化和端到端推理依赖张量形状、显存账本、KV cache 和批处理。分布式训练还需要理解前向、反向、梯度同步和参数更新之间的关系。PyTorch DistributedDataParallel（DDP，分布式数据并行）、FSDP（Fully Sharded Data Parallel，完全分片数据并行）和 Hugging Face 的缓存文档可作为实现参考。[^pytorch-ddp] [^pytorch-fsdp] [^hf-cache]
