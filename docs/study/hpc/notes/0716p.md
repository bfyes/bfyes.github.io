...
!!! abstract "与后续实验的联系"
    ...
    Lab 5 的量化和端到端推理依赖张量形状、显存账本、KV cache 和批处理。分布式训练还需要理解前向、反向、梯度同步和参数更新之间的关系。PyTorch DistributedDataParallel、FSDP 和 Hugging Face 的缓存文档可作为实现参考。[^pytorch-ddp] [^pytorch-fsdp] [^hf-cache]

当模型、批次（batch）或上下文长度超过单张 GPU 的显存和吞吐范围时，训练与推理需要在多张卡之间划分。数据并行（DP）沿 batch 切分数据。ZeRO/FSDP 分片模型状态。流水线并行（PP）按层切分。张量并行（TP）在层内切分矩阵。上下文并行沿序列切分。学习一种方案时，都要说明每张卡保存什么状态、执行什么计算、需要交换哪些数据。[^megatron] [^zero-paper]

## 一次训练 step 在多卡上怎样进行

=== "1. 每张卡取得自己的数据"

    数据并行时，每个 rank 取得不同的 micro-batch。模型副本或参数分片已经由运行时放在对应 GPU 上。输入张量的 batch 维被切开，模型层内的 shape 规则保持不变。

=== "2. 前向与反向计算"

    每张卡执行前向计算，得到 logits 和 loss。反向传播生成本地梯度。模型太大时，FSDP、TP 或 PP 会让参数和层计算分布在多张卡上，但仍遵循前向、loss、反向的顺序。

=== "3. 交换必要数据"

    DP 需要规约梯度。FSDP 在计算某层前聚合必要参数，计算后再次分片。TP 在层内交换激活或部分结果。PP 在相邻 stage 之间传递 activation 和梯度。

=== "4. 更新参数"

    参与同一同步训练步骤的 rank 使用一致的全局梯度或对应参数分片更新优化器状态。随后进入下一个 batch。通信没有消失，只是被安排在不同的时间和不同的张量上。
...
语言模型接收提示词（prompt）后，根据已有 token 预测下一个 token。新 token 会加入输入，生成持续到结束符。这种逐步生成称为自回归。token 是分词器切出的处理单位，可以是词片、汉字、标点或字节片段。embedding 将 token 编号映射为向量，常见输入形状为 $[B,S,H]$。
...
embedding 只给 token 一个可计算的向量表示，不包含词序。位置编码或位置嵌入补充它位于第几个位置。随后张量经过一层又一层 Transformer block。每层由 attention 和 FFN/MLP 等部分组成，最后经输出投影和 softmax 得到词表上的概率分布。
...
系统通常同时关注首 token 时间（TTFT）、后续 token 间延迟（TPOT）和总体吞吐。为了吞吐而积累很大的 batch，可能伤害交互请求的首 token 延迟。连续批处理、分页式 KV cache 和分开调度 prefill 与 decode 都用于协调这些目标。
...
!!! note "训练与推理的显存对象不同"
    ...
    推理通常需要参数和 KV cache。训练还需要梯度、优化器状态以及为反向传播保留或重算的 activation。看到“7B 模型的 BF16 权重约 14 GB”时，这只是权重文件占用的量级。训练显存还要按优化器、精度、batch、序列长度和实现策略继续计算。

## 通信中的点对点和集合操作

点对点通信（point-to-point, P2P）由一个 rank `send`、另一个 rank `recv` 组成。集合通信（collective communication）由一个通信组内的所有 rank 共同参加，例如 4 张 GPU 同时执行一次 AllReduce。NCCL、MPI、Gloo 等通信库会依据通信器、拓扑和消息大小选择具体算法。[^nccl-collectives]
...
*AllReduce 的输出复制到全部参与 rank，因此适合数据并行梯度同步。归约操作可以是 sum、max、min 等。训练中是否再除以 world size，取决于框架和优化器语义。* [^nccl-collectives]
...
checkpointing 可以用一条简单的时间线理解。普通训练在每个矩阵乘后保留中间结果，反向时直接使用。checkpointing 只保留某些边界处的输入，反向到该段时重算中间结果并接着求梯度。它减少激活显存，增加前向计算次数，通常不减少优化器状态。ZeRO/FSDP 则减少参数、梯度和优化器状态的副本或分片。
...
真实的大模型训练常把几种策略叠起来。节点内做 TP，按层做 PP，剩余维度做 DP，再用 ZeRO/FSDP 分片状态。这常被称为 3D 并行。阅读一组并行度配置时，可以把总 GPU 数写成各维度的乘积，再逐维检查下列问题。
...
以 GPT-3 175B 为例，可把 8 块 A100 通过 NVLink 组成一个节点作为 TP 组，8 个节点按层做 PP，再复制 60 份做 DP，得到 3840 块 GPU 的集群配置。每个维度都对应一类通信。TP 在节点内频繁 AllReduce，PP 在相邻 stage 之间传 activation/gradient，DP 需要全局 AllReduce 梯度。SP、CP 沿序列维切分，EP 则按专家切分并触发 All-to-All。面对具体配置时，应先算清 GPU 如何分组、组间传什么数据。
...
序列并行、FlashAttention 和 ring attention 都要处理 attention 的 $S^2$ 项。它既带来较大的计算量，也让中间 attention 矩阵和 KV cache 难以完整物化或驻留单卡。可以先区分 $S\ll H$ 与 $S>H$ 两种情况，再决定投影和 FFN 是否切分，或 attention 是否分块。
...
以 ring attention 为例，每张卡保存一段 Q/K/V。该类上下文并行实现将 attention 的全局依赖转化为分块计算与环形传输；数值稳定的分块 softmax 与 FlashAttention 的在线累积思想紧密相关。它先拿本地 Q 与本地 K/V 算一块 attention，然后把 K/V 块沿环传给下一个 rank，同时接收上一个 rank 的块；经过若干轮，每个 Q 块都和完整上下文的 K/V 做过计算。这样不必把所有 K/V 同时复制到每张卡，峰值内存随序列分片下降，代价是多轮环形通信。 [^flashattention]
...
Mixture of Experts（MoE）层用 router 为每个 token 选择少数几个 expert，而不是让所有 token 经过同一个 FFN。条件计算与路由负载均衡是大规模 MoE 系统的核心约束。总参数可以非常大，单个 token 的计算量却只与被选中的少数专家有关。这是 MoE 能扩大模型容量的原因。 [^gshard]

MoE 的额外开销主要来自路由和网络。某张卡上的 token 可能需要送去别张卡拥有的 expert，计算后结果还要按原 token 顺序送回，通常形成 All-to-All。router 若总是偏向少数专家，就会出现部分专家和 GPU 很忙、部分资源空闲的情况，因此需要容量限制和负载均衡损失。MoE 的瓶颈可能出现在 token 重排、All-to-All、专家不均衡或跨节点拓扑。[^gshard]
